...
<div id="main" style="overflow-x:scroll;overflow-y:scroll;width:78%;height:90%">
		<h2 align="center" class="style1">Internal Calls</h2>
...
if ($select2 != ''){
$query = "SELECT COUNT(day) FROM internal WHERE $select1 = '$param1' AND $select2 LIKE '$param2'AND $select3 = '$param3' ORDER BY id ASC"; 
$result = mysqli_query($con, $query);
}
else {
$query = "SELECT COUNT(day) FROM internal WHERE $select1 = '$param1'AND $select3 = '$param3' ORDER BY id ASC";
$result = mysqli_query($con, $query);
}
...
$param3= $_GET['selectvar3'];
if ($select2 != ''){
$query = "SELECT * FROM internal WHERE $select1 = '$param1' AND $select2 LIKE '$param2'AND $select3 = '$param3' ORDER BY id ASC"; 
$result = mysqli_query($con, $query);
}
else {
$query = "SELECT * FROM internal WHERE $select1 = '$param1'AND $select3 = '$param3' ORDER BY id ASC";
$result = mysqli_query($con, $query);
}
...
<table style=" border:1px solid silver" cellpadding="5px" cellspacing="1px" border="1"> 
  <tr>
    ...
    <td nowrap><div align="center"> <font size="2"><strong>Caller Ext.</strong></font></div></td>
    <td nowrap><div align="center"> <font size="2"><strong>Called Ext.</strong></font></div></td>
...
$codetest = (substr(($row->called),0,1));
...
$extquery = "SELECT `first` FROM `extension` WHERE `extension` = '{$row->caller}'";
$extresult = mysqli_query($extquery);
$extprint = mysqli_fetch_array($extresult);
$first = $extprint['first'];
$extquery2 = "SELECT `first` FROM `extension` WHERE `extension` = '{$row->called}'";
$extresult2 = mysqli_query($extquery2);
$extprint2 = mysqli_fetch_array($extresult2);
$first2 = $extprint2['first'];
...
?>
  <tr> 
    ...
    <td><div align="center"><font size="2"><?php echo ($row->time);?></font></div></td>
    <td><div align="center"><font size="2"><?php echo $first;?> <?php echo ($row->caller);?></a></font></div></td>
    <td><div align="center"><font size="2"><?php echo $first2;?> <?php echo ($row->called);?></font></div></td>
	<td><div align="center"><font size="2"><?php echo ($row->dur); ?></font></div></td>
  </tr>
<?php
}
?>
